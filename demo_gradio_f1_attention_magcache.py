...
@torch.no_grad()
def worker(input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_magcache, mp4_crf,resolution=320, imagemode=True, savepng=True, fps2430=24, use_teacache=False, magcache_thresh=0.1, magcache_K=3, magcache_retention_ratio=0.2):
...
def process(input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_magcache, mp4_crf,resolution, imagemode, savepng, fps2430, use_teacache=False, magcache_thresh=0.1, magcache_K=3, magcache_retention_ratio=0.20):
    global stream
    ...
    async_run(worker, input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_magcache, mp4_crf,resolution, imagemode, savepng, fps2430, use_teacache, magcache_thresh, magcache_K, magcache_retention_ratio)
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Group():
                ...
                n_prompt = gr.Textbox(label="NegativeNegative Prompt", value="", visible=True)  # Not used
                
                # Auto-generate a 5-digit seed
                seed_auto = random.randint(10000, 99999)
                seed = gr.Number(label="Seed", value=seed_auto, precision=0)
    ...
    ips = [input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_magcache, mp4_crf, resolution, imagemode, savepng, fps2430, use_teacache, magcache_thresh, magcache_K, magcache_rention_ratio]
