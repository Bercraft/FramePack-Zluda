...
title FramePack-Zluda Installer
...
cls
echo ---------------------------------------------------------------
Echo * FramePack-ZLUDA INSTALL (for HIP 6.2.4 with MIOPEN and Triton)*
echo ---------------------------------------------------------------
echo.
...
pip install triton-3.0.0+git2b248f14-cp311-cp311-win_amd64.whl

echo  ::  %time:~0,8%  ::  - Installing flash-attention

pip install flash_attn-2.7.4.post1-py3-none-any.whl --quiet
copy customzluda\fa\distributed.py %VIRTUAL_ENV%\Lib\site-packages\flash_attn\utils\distributed.py /y >NUL

pip install sageattention --quiet
echo  ::  %time:~0,8%  ::  - Patching sage-attention
copy customzluda\sa\quant_per_block.py %VIRTUAL_ENV%\Lib\site-packages\sageattention\quant_per_block.py /y >NUL
copy customzluda\sa\attn_qk_int8_per_block_causal.py %VIRTUAL_ENV%\Lib\site-packages\sageattention\attn_qk_int8_per_block_causal.py /y >NUL
copy customzluda\sa\attn_qk_int8_per_block.py %VIRTUAL_ENV%\Lib\site-packages\sageattention\attn_qk_int8_per_block.py /y >NUL
